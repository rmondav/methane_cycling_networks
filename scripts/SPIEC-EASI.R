...
MC_anO2_lake_clean_otus=otu_table(MC_anO2_lake_clean_psob)
...
MC_anO2_lake_spieceasi <-spiec.easi(
  MC_anO2_lake_clean_otus, method='mb', lambda.min.ratio=0.5, 
  nlambda=50, sel.criterion='stars', pulsar.select=TRUE, pulsar.params=paramargs)
saveRDS(MC_anO2_lake_spieceasi, "./MC_anO2_lake_spieceasi.rds")
#MC_anO2_lake_spieceasi <- readRDS("./MC_anO2_lake_spieceasi.rds")


MC_anO2_pond_clean_psob<-import_biom(
  "./OTU_table_anO2_pond_cultures_2000_min11S.ovr30count.biom", 
  parseFunction=parse_taxonomy_default)
MC_anO2_pond_clean_otus=otu_table(MC_anO2_pond_clean_psob)
MC_anO2_pond_clean_taxa=tax_table(MC_anO2_pond_clean_psob)
#run spiec.easi
MC_anO2_pond_spieceasi <-spiec.easi(
  MC_anO2_pond_clean_otus, method='mb', lambda.min.ratio=0.5, 
  nlambda=50, sel.criterion='stars', pulsar.select=TRUE, pulsar.params=paramargs)
...
saveRDS(MC_anO2_pond_spieceasi, "./MC_anO2_pond_spieceasi.rds")

MC_O2_lake_clean_psob<-import_biom(
  ".OTU_table_O2_lake_cultures_2000_min11S.ovr30count.biom.w_taxonomy.txt", 
  parseFunction=parse_taxonomy_default)
MC_O2_lake_clean_otus=otu_table(MC_O2_lake_clean_psob)
MC_O2_lake_clean_taxa=tax_table(MC_O2_lake_clean_psob)
MC_O2_lake_spieceasi <-spiec.easi(
  MC_O2_lake_clean_otus, method='mb', lambda.min.ratio=0.5, 
  nlambda=50, sel.criterion='stars', pulsar.select=TRUE, pulsar.params=paramargs)
saveRDS(MC_O2_lake_spieceasi, "./MC_O2_lake_spieceasi.rds")

TS_lake_clean_psob<-import_biom(
  ".OTU_table_TS_lake_17000_min18S.ovr189count.biom.w_taxonomy.txt", 
  parseFunction=parse_taxonomy_default)
TS_lake_clean_otus=otu_table(TS_lake_clean_psob)
TS_lake_clean_taxa=tax_table(TS_lake_clean_psob)
TS_lake_spieceasi <-spiec.easi(
  TS_lake_full, method='mb', lambda.min.ratio=0.5, nlambda=50, 
  sel.criterion='stars', pulsar.select=TRUE, pulsar.params=paramargs)
saveRDS(TS_lake_spieceasi, "./TS_lake_spieceasi.rds")
